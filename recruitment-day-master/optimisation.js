  ...
  app.update = function (timestamp) {
    for (var i = 0; i < app.count; i++) {
      ...
      if (!app.optimize) {
        ...
        var top = m.classList.contains('down') ? m.style.top + distance : m.style.top - distance;
        var left = m.classList.contains('right') ? m.getBoundingClientRect().left + distance : m.getBoundingClientRect().left - distance;
        if (top < 0) top = 0;
        if (top > maxHeight) top = maxHeight;
        m.style.top = top + 'px';
        if (left < 0) left = 0;
        ...
        if (m.getBoundingClientRect().left === 0) {
          m.classList.remove('left');
          m.classList.add('right');
        }
        if (m.getBoundingClientRect().left === maxWidth) {
          m.classList.remove('right');
          m.classList.add('left');
        }
      }
